    ...
    <div class="max-w-lg mx-auto bg-white p-6 rounded-md shadow-md">
        ...
        <form method="POST" action="{{ isset($student) ? route('students.update', $student->id) : route('students.store') }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="w-full px-3 py-2 border rounded @error('name') border-red-500 @enderror" value="{{ old('name', $student->name ?? '') }}" required>
                @error('name')
            ...
            <div class="mb-4">
                ...
                <input type="email" name="email" id="email" class="w-full px-3 py-2 border rounded @error('email') border-red-500 @enderror" value="{{ old('email', $student->email ?? '') }}" required>
                @error('email')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="phone" id="phone" class="w-full px-3 py-2 border rounded @error('phone') border-red-500 @enderror" value="{{ old('phone', $student->phone ?? '') }}" required>
                @error('phone')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="dob" id="dob" class="w-full px-3 py-2 border rounded @error('dob') border-red-500 @enderror" value="{{ old('dob', $student->dob ?? '') }}" required>
                @error('dob')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                @enderror
            </div>
